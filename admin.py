from tkinter import *
import datetime
from subprocess import call



def create_account():
    call(["python","create_account.py"])

def balance_enquiry():
    call(["python","balance_enquiry.py"])

def check_account_details():
    call(["python","check_account_details.py"])

def withdraw_amount():
    call(["python","withdraw_amount.py"])

def deposit_amount():
    call(["python","deposit_amount.py"])

def delete_account():
    call(["python","delete_account.py"])

def update_account():
    call(["python","update_account.py"])

def user_administration():
    call(["python","user_administration.py"])




date = datetime.datetime.now().date()
date= str(date)

class application:
    def __init__(self,master):
        self.master=master

        #frames
        self.top=Frame(master,height=140,bg="white")
        self.top.pack(fill=X)

        self.bottom=Frame(master,height=580,bg="pink")
        self.bottom.pack(fill=X)

        #frame labels
        self.image1=PhotoImage(file='icon\money.png')
        self.image1lbl=Label(self.top, image=self.image1, bg="white")
        self.image1lbl.place(x=490,y=15)

        self.image2=PhotoImage(file='icon\money.png')
        self.image2lbl=Label(self.top, image=self.image2, bg="white")
        self.image2lbl.place(x=865,y=15)

        self.heading=Label(self.top,text=" Golden Horizon Bank",font="arial 15 bold",bg="white")
        self.heading.place(x=600,y=30)

        self.date_lbl=Label(self.bottom,text="Date : "+date,bg="pink")
        self.date_lbl.place(x=650,y=20)

        #buttons and labels

        #create account
        self.ca=Button(self.bottom,text="Create Account",font="arial 13 bold",width=20,command=create_account)
        self.ca.place(x=40, y=50)
        self.ca_lbl=Label(self.bottom,text="> Create new user account",bg="pink",font="arial 13 bold")
        self.ca_lbl.place(x=280, y=52)

        #balance enquiry
        self.be=Button(self.bottom,text="Balance Enquiry",font="arial 13 bold",width=20,command=balance_enquiry)
        self.be.place(x=40, y=120)
        self.be_lbl=Label(self.bottom,text="> Check account balance",bg="pink",font="arial 13 bold")
        self.be_lbl.place(x=280, y=122)

        #check account details
        self.cad=Button(self.bottom,text="Check Account Details",font="arial 13 bold",width=20,command=check_account_details)
        self.cad.place(x=40, y=190)
        self.cad_lbl=Label(self.bottom,text="> Check account details",bg="pink",font="arial 13 bold")
        self.cad_lbl.place(x=280, y=192)

        #Withdraw Amount
        self.wa=Button(self.bottom,text="Withdraw Amount",font="arial 13 bold",width=20,command=withdraw_amount)
        self.wa.place(x=40, y=260)
        self.wa_lbl=Label(self.bottom,text="> Withdraw amount from user account",bg="pink",font="arial 13 bold")
        self.wa_lbl.place(x=280, y=262)

        #Deposit Amount
        self.da=Button(self.bottom,text="Deposit Amount",font="arial 13 bold",width=20,command=deposit_amount)
        self.da.place(x=40, y=330)
        self.da_lbl=Label(self.bottom,text="> Deposit amount from user account",bg="pink",font="arial 13 bold")
        self.da_lbl.place(x=280, y=332)

        #delete account
        self.delacc=Button(self.bottom,text="Delete Account",font="arial 13 bold",width=20,command=delete_account)
        self.delacc.place(x=40, y=400)
        self.delacc_lbl=Label(self.bottom,text="> Delete user account",bg="pink",font="arial 13 bold")
        self.delacc_lbl.place(x=280, y=402)

        #update account
        self.updacc=Button(self.bottom,text="Update Account",font="arial 13 bold",width=20,command=update_account)
        self.updacc.place(x=40, y=470)
        self.updacc_lbl=Label(self.bottom,text="> Update user account",bg="pink",font="arial 13 bold")
        self.updacc_lbl.place(x=280, y=472)

        #user administration
        self.usrad=Button(self.bottom,text="User Administration",font="arial 13 bold",width=20,command=user_administration)
        self.usrad.place(x=40, y=540)
        self.usrad_lbl=Label(self.bottom,text="> Actions related to users",bg="pink",font="arial 13 bold")
        self.usrad_lbl.place(x=280, y=542)

        #logout
        self.log=Button(self.bottom,text="Logout",font="arial 13 bold",width=7,command=quit)
        self.log.place(x=1200,y=20)
        
        




root=Tk()
root.title("BANK MANAGEMENT SYSTEM")
app=application(root)
root.geometry("1366x768")
root.resizable(True,True)
root.mainloop()